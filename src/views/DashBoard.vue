  ...
  <div class="container">
    ...
    <div v-else>
      ...
      <div v-else>
          ...
          <footer v-if="this.liveEvent.submissions">
            <fieldset role="group" v-if="!this.submission.uploaded">
              ...
              <input
                ...
                :aria-busy="this.submission.loading"
                :disabled="this.submission.loading"
              />
            </fieldset>
            <p v-else>
              <a :href="this.submission.fileURL" target="_blank" class="btn"
        ...
        <RegisterModal
          @done="(d) => this.userData.events.push(d)"
          :open="this.modalOpen"
        ...
        <div class="grid" style="position: relative">
          <div v-if="this.unRegisteredEvents.length > 0">
            ...
            <ul class="event-list">
              <li
                v-for="event in this.unRegisteredEvents"
                :key="event.id"
                class="reg-event-card"
              >
                <article>
                  <header class="event-header">{{ event.name }}</header>
                  <header class="mark-tag">
                    <mark
                      >Participation Type:
                      {{ event.rule[0].participation }}</mark
                    >
                  </header>
                  ...
                  <footer>
                    ...
                    <router-link :to="'/rulebook/' + event.id" class="link-reg"
          ...
          <div style="position: static">
            <h3 style="width: 100%; color: teal;">Your Registration Details</h3>
            <table>
              <tbody>
                <tr>
                  <td>Email</td>
            ...
            <div v-if="this.userEvents.length > 0">
              ...
              <ul class="event-list">
                <li v-for="event in this.userEvents" :key="event.id" class="reg-event-card">
                  <article>
                    <header class="event-header">{{ event.name }}</header>
                    <header class="mark-tag">
                      <mark>{{
                        event.confirmed ? "Confirmed" : "Pending Approval"
                      }}</mark>
                    </header>
                    <p>{{ event.rule[0].overview[0].des }}</p>
                  </article>
            ...
            <div v-if="this.userData.submissions">
              ...
              <ul v-if="this.liveEvent">
                <li
                  v-for="(submission, i) in this.userData.submissions"
                  :key="i"
                >
                  <SubmissionBox
                    :sub="submission"
                    :liveEvent="this.liveEvent.id"
                    @submittedForLive="
                      (e) => {
                        this.submission.uploaded = true;
                        this.submission.fileURL = e;
                      }
                    "
                  />
                </li>
...
.reg-event-card {
  ...
  box-shadow: 0 8px 32px 0 rgba(0, 128, 128, 0.3);
  z-index: 10;
...
export default {
    ...
    userEvents() {
      ...
      for (let i in this.userData.events) {
        let event = this.events.find(
          (e) => e.id == this.userData.events[i].event_id
        );
        if (event) {
          out.push({
            ...event,
  ...
  methods: {
    ...
    uploadFile() {
      ...
      let storageRef = ref(
        storage,
        `submissions/${event}/${this.user.email}/${file.name}`
      );
      if (
        !this.isEventLive ||
        !this.isRegforcurrentEvent ||
        !file ||
        !this.liveEvent.submissions
      ) {
        return;
      }
